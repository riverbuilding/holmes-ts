...
export function renderResult(result: InvestigationResult): string {
  ...
  const status = result.complete
    ? "Status: complete"
    : `Status: partial — ${partialReasonLabel(result.reason)}`;
  ...
  const warnings = [
    ...
    !validation.hasCitations && result.evidence.length > 0
      ? "- No evidence citations were supplied despite retained evidence."
      : undefined
  ].filter((warning): warning is string => warning !== undefined);
  const validatedCitations = validation.validEvidenceIds.length === 0
    ? ""
...
function partialReasonLabel(reason: InvestigationStopReason | undefined): string {
  ...
  switch (reason) {
    case "deadline": return "investigation deadline reached";
    case "tool-limit": return "tool-call limit reached";
    case "model-limit": return "model-call limit reached";
    case "duplicate-only": return "only duplicate tool requests remained";
    case "cancelled": return "investigation cancelled";
    case "provider-error": return "model provider failed";
  }
}
